...
<h2>收费细项</h2>
    <form method="POST" action="updatecharge.do">
      <p>收费项目：<select size="1" name="chargename">
        <c:forEach items="${list}" var="c">
        	<option value="${c.chargecode}">${c.chargename}(${c.charge})</option>
        </c:forEach>
        
      </select></p>
      <p>收费金额：<input type="text" name="charge" size="20"></p>
      <p><input type="submit" value="提交" name="B1"><input type="reset" value="全部重写" name="B2"></p>
    </form>
    <hr>
    <h2>业务收费</h2>
    <form method="POST" action="updatechargerule.do">
      <p>业务：<select size="1" name="func_name">
        <c:forEach items="${listr}" var="cr">
        	<option value="${cr.func_name}">${cr.func_name}</option>
        </c:forEach>
      </select></p>
      ...
      <input type="checkbox" name="C4" value="ON" checked>入网费 <input type="checkbox" name="C5" value="ON" checked>选号费 -->
      <c:forEach items="${list}" var="c">
        	 <input type="checkbox" name="chargecontent" value="${c.chargecode}">${c.chargename} 
        </c:forEach>
